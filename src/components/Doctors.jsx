...
export default function Doctors() {
  ...
  const visible =
    filter === 'All' ? doctors : doctors.filter((doctor) => doctor.specialty === filter)
  const active = visible.find((doctor) => doctor.id === activeId) ?? visible[0]
  const activeIndex = active ? visible.findIndex((doctor) => doctor.id === active.id) : 0

  function selectFilter(value) {
    setFilter(value)

    const next =
      value === 'All' ? doctors[0] : doctors.find((doctor) => doctor.specialty === value)
  ...
  if (loading) {
    roster = (
      <div className="mt-8 grid min-h-[300px] animate-pulse overflow-hidden rounded-[24px] border border-navy/10 bg-navy/90 sm:mt-12 sm:min-h-[380px] sm:rounded-[36px] md:min-h-[520px]" />
    )
  } else if (error) {
    roster = (
      <div className="mt-8 rounded-[24px] border border-navy/10 bg-pale px-6 py-10 text-center sm:mt-12">
        <p className="text-sm text-navy/70">Specialists could not be loaded right now.</p>
        <Button as="button" variant="secondary" className="mt-4" onClick={loadDoctors}>
          Try again
        </Button>
      </div>
    )
    ...
    roster = (
      <div className="mt-8 rounded-[24px] border border-navy/10 bg-pale px-6 py-10 text-center sm:mt-12">
        <p className="text-sm text-navy/70">No specialists are available at the moment.</p>
      </div>
    )
  } else {
    roster = (
        ...
        <div className="flex max-h-[320px] flex-col overflow-y-auto border-t border-white/10 md:max-h-none md:border-l md:border-t-0">
          ...
          <div className="flex-1">
            {visible.map((doctor, index) => {
              const selected = doctor.id === active.id
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`relative z-10 text-[12px] font-semibold tracking-[0.16em] ${
                      selected ? 'text-mint' : 'text-white/30'
                    }`}
                  >
                    {pad(index)}
                  </span>
                  ...
                  {selected ? <span className="relative z-10 h-1.5 w-1.5 rounded-full bg-mint" /> : null}
                </button>
              )
            })}
          </div>
